...
request(environment.apiURL + 'trips', {json: true}, function (error, response, body) {
  if (!error) {
    const tripIds = body['trips'].map(trip => trip.id);
    const tripWithReportIds = body['trips'].filter(trip => trip.trip_report).map(trip => trip.id);
    tripIds.forEach(id => ROUTES.push('/trips/' + id));
    tripWithReportIds.forEach(id => ROUTES.push('/trips/' + id + '/report'));
